...
def isalpha(uni_ch):
    """判断一个 unicode 是否是字母。"""
    if (re.search("[\u0041-\u005a]", uni_ch)) \
       or (re.search("[\u0061-\u007a]", uni_ch)):
        return True
    else:
        return False
...
def add_spaces_to_string(string, code):
    ...
    newustr = ""
    flag = 0
    ustr = string.encode(code).decode()
    ch_lst = list(ustr)
    length = len(ch_lst)
    for i in range(0, length):
        ...
        newustr += ch_lst[i]
    newstring = newustr.encode(code).decode()
    if flag == 1:
        #处理中文里的粗体字和斜体字
